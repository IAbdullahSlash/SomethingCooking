...
export default function AnalysisPage() {
  ...
  useEffect(() => {
    const loadOrGenerateAnalysis = async () => {
      ...
      const pendingIdea = localStorage.getItem("pendingProjectIdea")
      if (pendingIdea) {
        console.log("[v0] Found pending idea:", pendingIdea.substring(0, 50) + "...")
        try {
          console.log("[v0] Making API call to /api/analyze...")
          ...
          if (response.ok) {
            ...
            const enhancedAnalysis = {
              ...analysisData,
              projectTitle: `Project: ${pendingIdea.substring(0, 50)}${pendingIdea.length > 50 ? "..." : ""}`,
              projectDescription: pendingIdea,
              projectDomain: "Web Development",
            }

            setAnalysis(enhancedAnalysis)
            localStorage.setItem("projectAnalysis", JSON.stringify(enhancedAnalysis))
            localStorage.removeItem("pendingProjectIdea") // Clean up
            setProjectModifications({
            ...
            alert(`Failed to generate analysis. Status: ${response.status}. Check console for details.`)
          }
        } catch (error) {
          ...
          alert(`Network error occurred: ${errorMessage}. Check console and terminal for details.`)
        }
      }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex gap-4 justify-center pt-8">
            ...
            <Button size="lg" onClick={exportToPDF} disabled={exportLoading} className="flex items-center gap-2">
              {exportLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
